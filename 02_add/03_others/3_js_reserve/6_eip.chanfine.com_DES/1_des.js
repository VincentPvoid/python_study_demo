...
function getPwd(pwd,type) {
  var str = '4CFFA8D128FA75130208B4BAA7A4BA81';
  if (str.length < 32) {
    str += "abcdefghijklmnopqrstuvwxyz1234567890"
  }
  str = str.toUpperCase();
  var keyObj = {};
  keyObj.key = str.substring(0, 16);
  keyObj.iv = str.substring(16, 32);
  keyObj.security = "\u4435\u5320\u4d35"

  // var keyObj = {};
  if (type == null || "aes" == type.toLowerCase()) {
    // keyObj = SECURITYKEY.get();
    value = CryptoJS.AES.encrypt(pwd, CryptoJS.enc.Utf8.parse(keyObj.key), {
      iv: CryptoJS.enc.Utf8.parse(keyObj.iv)
    }).toString()
  } else {
    // keyObj = SECURITYKEY.get('des');
    value = CryptoJS.DES.encrypt(pwd, CryptoJS.enc.Hex.parse(keyObj.key), {
