...
# 获取目标视频pid参数
def get_target_pid():
  url = "https://tv.cctv.com/v/v1/VIDEDE9UYv92Y1VxQrg31tuu100506.html"
  res = requests.get(url)
  # with open('index.html', mode="wb") as fp:
  #   fp.write(res.content)
  regObj = re.compile(r"var video_guid='(.)+guid", re.S)
  temp = regObj.search(res.text).group()
  # print(temp)
  pid = temp.replace("var video_guid='", '').split("';//")[0]
  # print(pid)
  return pid


# 获取视频入口地址
def get_index_url(pid):
  # 视频信息请求地址
...
def main():
  ...
  pid = get_target_pid()
  # 视频对应入口地址
  page_url = get_index_url(pid)
  # 入口m3u8文件
  m3u8_file_url = get_m3u8_url(page_url)
